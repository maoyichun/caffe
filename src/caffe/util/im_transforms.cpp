...
void AdjustSaturation(const cv::Mat& in_img, const float delta,
  ...
  if (fabs(delta - 1.f) != 1e-3) {
	...
	//printf("AdjustSaturation cvtColor done!\n");
...
void RandRotateImage(const cv::Mat& in_img, cv::Mat* out_img, const float rotate_degree, const float rotate_prob)
{
	float prob;
	caffe_rng_uniform(1, 0.f, 1.f, &prob);
	if (prob < rotate_prob) {
		CHECK_GE(rotate_degree, 0) << "rotate_degree must be non-negative.";
		float degree;
		caffe_rng_uniform(1, -rotate_degree, rotate_degree, &degree);

		CvPoint2D32f center;
		center.x = float(in_img.cols / 2.0 + 0.5);
		center.y = float(in_img.rows / 2.0 + 0.5);
		
		//float map[6];
		cv::Mat map_matrix;// = cv::Mat(2, 3, CV_32F, map);
		//CvMat map_matrix2 = map_matrix;
		//cv2DRotationMatrix(center, degree, 1.0, &map_matrix2);
		//cv::warpAffine(in_img, *out_img, map_matrix, cv::Size(in_img.rows, in_img.cols), 1, 0, cvScalar(128));
        map_matrix = cv::getRotationMatrix2D(center, degree, 1.0);
        cv::warpAffine(in_img, *out_img, map_matrix, in_img.size(), 1, 0, cv::Scalar(128));
	} else {
		*out_img = in_img;
	}
}
...
cv::Mat ApplyDistort(const cv::Mat& in_img, const DistortionParameter& param) {
  ...
  if (prob > 0.5) {
	...
	//printf("RandomSaturation done!\n");
    // Do random hue distortion.
    RandomHue(out_img, &out_img, param.hue_prob(), param.hue_delta());
	//printf("RandomHue done!\n");
    // Do random reordering of the channels.
    RandomOrderChannels(out_img, &out_img, param.random_order_prob());
	//printf("RandomOrderChannels done!\n");
	//printf("RandRotateImage start!\n");
	RandRotateImage(out_img, &out_img, param.rotate_degree(), param.rotate_prob());
	//printf("RandRotateImage done!\n");
  } else {
    // Do random brightness distortion.
    RandomBrightness(out_img, &out_img, param.brightness_prob(),
                     param.brightness_delta());
...
cv::Mat ApplyObjScale(const cv::Mat& in_img, const ObjectScaleParameter& param)
{
	cv::Mat out_img = in_img;
	float prob;
	caffe_rng_uniform(1, 0.f, 1.f, &prob);
	...
	int max_piex;
	min_scale = param.min_scale();
	max_scale = param.max_scale();
	max_piex = param.max_piex();
	...
	CHECK_GE(min_scale, 0) << "min scale must be non-negative.";
	...
	if(prob < param.prob())
	{
		...
		if(scale > 1.0)
		{
			...
			offset_h = rand()%(new_h - ori_h + 1);
			offset_w = rand()%(new_w - ori_w + 1);

			cv::Mat mNew = cv::Mat(new_h, new_w, CV_8UC3, cv::Scalar(128,128,128));
			cv::Rect roi(offset_w, offset_h, ori_w, ori_h);
			in_img.copyTo(mNew(roi));

			cv::resize(mNew, out_img, cv::Size(ori_w, ori_h), 0, 0);
		}
		else
		{
			int offset_w_1, offset_h_1;
			int offset_w_2, offset_h_2;
			offset_w_1 = rand() % max_piex;
			offset_h_1 = rand() % max_piex;
			offset_w_2 = rand() % max_piex;
			offset_h_2 = rand() % max_piex;
			
			cv::Rect roi(offset_w_1, offset_h_1, ori_w - offset_w_1 - offset_w_2, ori_h - offset_h_1 - offset_h_2);
			cv::Mat mTemp = in_img(roi).clone();
			cv::resize(mTemp, out_img, cv::Size(ori_w, ori_h), 0, 0);
		}
	}
	else
	{
